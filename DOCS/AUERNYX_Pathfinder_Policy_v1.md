# Auernyx Pathfinder Policy v1

**ID:** `AUERNYX.Pathfinder.Policy.v1`  
**Purpose:** Provide a consistent, veteran-first guidance layer above SQUAD modules.

This file defines what the Pathfinder layer **may do**, **must do**, and **must refuse**. It is intentionally short and strict.
...
## 7) Standard Pathfinder output format (required)
...
Pathfinder supports **review-oriented modes** (truth review, not authorship) when the input envelope sets:
...
In **case review modes**, Pathfinder must be stricter than the default guidance format:
