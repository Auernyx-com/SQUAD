"""Validate Pathfinder Contract v1 JSON envelopes.
...
def _repo_root() -> Path:
    # tools/qa/validate_pathfinder_contracts.py -> repo root is parents[2]
...
def _basic_validate_contract(doc: Any, *, file_path: Path) -> List[str]:
    ...
    if doc.get("contract_id") != "AUERNYX.Pathfinder.Contract.v1":
        errs.append("contract_id must equal 'AUERNYX.Pathfinder.Contract.v1'")
...
def main(argv: Sequence[str]) -> int:
    parser = argparse.ArgumentParser(description="Validate Pathfinder Contract v1 envelopes.")
    parser.add_argument(
        "--root",
    ...
    parser.add_argument(
        "--schema",
        default=str(_repo_root() / "AGENTS" / "SCHEMAS" / "Pathfinder_Contract_v1.schema.json"),
        help="Path to Pathfinder Contract v1 schema",
    )
    ...
    print(f"pathfinder contract files scanned: {total}")
