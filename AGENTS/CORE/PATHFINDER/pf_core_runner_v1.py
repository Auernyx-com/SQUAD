...
_CONTRACT_ID = "AUERNYX.Pathfinder.Contract.v1"
_SCHEMA_RELATIVE = ("SCHEMAS", "Pathfinder_Contract_v1.schema.json")

from pf_truth_v1 import TruthAssessment, assess_truth
...
def build_pathfinder_plan(input_env: Dict[str, Any]) -> Tuple[PathfinderDraft, TruthAssessment]:
    case = (input_env or {}).get("case") or {}
...
def build_case_review_plan(input_env: Dict[str, Any], mode: str) -> Tuple[PathfinderDraft, TruthAssessment, List[Dict[str, Any]], List[str]]:
    case = (input_env or {}).get("case") or {}
    flags = (input_env or {}).get("flags") or {}
    artifacts = (input_env or {}).get("artifacts") or []
...
def _schema_path() -> Path:
    """Locate the Pathfinder Contract v1 schema relative to this module.

    pf_core_runner_v1.py lives at AGENTS/CORE/PATHFINDER/;
    the schema is at AGENTS/SCHEMAS/Pathfinder_Contract_v1.schema.json,
...
def _validate_contract_schema(payload: Dict[str, Any]) -> None:
    """Validate *payload* against the Pathfinder Contract v1 JSON schema.
...
def run(input_path: Path, output_path: Optional[Path] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        if mode:
            ...
            plan, truth = build_pathfinder_plan(input_env)
            warnings = []
            mode_updates = []
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Auernyx Pathfinder PF-Core minimal runner (v1).")
