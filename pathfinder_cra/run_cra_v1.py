...
def _repo_root_from_here() -> Path:
    # pathfinder_cra/run_cra_v1.py -> repo root
...
def _paths() -> CraPaths:
    ...
    return CraPaths(
        repo_root=root,
        schema_in=root / "pathfinder_cra" / "schema" / "cra.schema.json",
        schema_out=root / "pathfinder_cra" / "schema" / "cra_output.schema.json",
    )
...
def _load_handshake_module(repo_root: Path):
    """Load PathfinderHandshake without requiring package installs.
    ...
    hs_path = repo_root / "AGENTS" / "CORE" / "PATHFINDER" / "pf_handshake_v1.py"
    if not hs_path.exists():
        raise FileNotFoundError(f"Handshake module not found at: {hs_path}")

    spec = importlib.util.spec_from_file_location("pf_handshake_v1", str(hs_path))
    if spec is None or spec.loader is None:
        raise ImportError("Failed to create import spec for handshake module")

    mod = importlib.util.module_from_spec(spec)
    sys.modules["pf_handshake_v1"] = mod
    spec.loader.exec_module(mod)

    if not hasattr(mod, "PathfinderHandshake") or not hasattr(mod, "BBOutput"):
        raise ImportError("Handshake module missing expected exports: PathfinderHandshake / BBOutput")

    return mod


def _cra_handshake_questions() -> list[str]:
    # Schema-aligned questionnaire for pathfinder_cra/schema/cra.schema.json.
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Pathfinder CRA runner (v1) — process-only schema-driven report generator (no fetches)."
    )
